...
class ligand_Featurization():
    def __init__(self,model_select='chembl27_512'):
        ...
        self.model_path = os.path.join('PretrainModels', model_select)
        self.dict_file = os.path.join(self.model_path, "dict.txt")
        self.generate = os.path.join('PretrainModels', 'bt_pro', 'generate_bt_fps_mean.py')


    def process(self,smi_file,feature_file):
        
        # cwd = 'python "%s"  --model_name_or_path "%s" ...'%(self.generate, self.model_path, ...)

        cwd = 'python "{}" --model_name_or_path "{}" --checkpoint_file checkpoint_best.pt --data_name_or_path "{}" --dict_file "{}" --target_file "{}" --save_feature_path "{}"'.format(self.generate, self.model_path, self.model_path, self.dict_file, smi_file, feature_file)
...
os.remove('temp-transformer.smi')
